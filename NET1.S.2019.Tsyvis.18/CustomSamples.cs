...
namespace QuerySamples
{
    ...
    public class CustomSamples  : SampleHarness
    {
        ...
        public void Linq1()
        {
            ...
            var customerGroup = from customer in customerList
                                let orderSum = customer.Orders.Sum(o => o.Total)
                                where orderSum > 10000
                                select new { customer.CustomerID, orderSum };

            foreach (var customer in customerGroup)
            {
                WriteLine($"Customer ID: {customer.CustomerID},  Sum: {customer.orderSum}");
            }
        }

        [Category("Tasks")]
        [Title("Task2-With AnonymousType")]
        ...
        public void Linq2WithAnonymousType()
        {
            List<Customer> customers = GetCustomerList();

            var customerGroups = from customer in customers
                                 select new { customer, supplierList =
                                 from supplier in GetSupplierList()
            ...
            {
                WriteLine($"{item.customer.CustomerID} {item.customer.Country} {item.customer.City}:");
                foreach (var element in item.supplierList)
                {
                    WriteLine($"      Supplier name: {element.supplier.SupplierName}");
                }
            }

            WriteLine(customerGroups.Count());
        }

        //[Category("Tasks")]
        //[Title("Task2")]
        //[Description("Для каждого клиента получить список поставщиков, находящихся в той же стране и том же городе. Задание выполнить, как используя операцию группировки, так и без нее")]
        //public void Linq2Simple()
        //{
        //    var customers = GetCustomerList();
        //    var suppliers = GetSupplierList();

        //    var customerGroups = from cust in customers
        //                         from supplier in suppliers
        //                         where supplier.City == cust.City && supplier.Country == cust.Country
        //                         select new { cust.CustomerID, supplier.Country, supplier.City };

        //    foreach (var customer in customerGroups)
        //    {
        //        WriteLine($"Customer ID: {customer.CustomerID}");
        //        WriteLine($"Counry: {customer.Country}, City: {customer.City}");
        //    }
        //}

        [Category("Tasks")]
        [Title("Task2-With GroupJoin")]
        [Description("Для каждого клиента получить список поставщиков, находящихся в той же стране и том же городе. Задание выполнить, как используя операцию группировки, так и без нее")]
        public void Linq2WithGroupJoin()
        {
            var customers = GetCustomerList();
            var suppliers = GetSupplierList();
            ...
            var customerGroups =
                ...
                join supplier in suppliers on new { customer.Country, customer.City } equals new { supplier.Country, supplier.City } into supplierList
                select new { supplierList, customer };
            ...
            foreach (var item in customerGroups)
            {
                ...
                foreach (var element in item.supplierList)
                {
                    WriteLine($"      Supplier name: {element.SupplierName}");
                }
            }
        ...
        public void Linq3()
        {
            var customerGroups = from customer in GetCustomerList()
                                 where customer.Orders.All(o => o.Total > 200)
                                 select customer;

            ObjectDumper.Write(customerGroups);
        }
        ...
        public void Linq4()
        {
            var sortedCustomers = from customer in GetCustomerList()
                                  orderby customer.Orders.FirstOrDefault()?.OrderDate.Year, customer.Orders.FirstOrDefault()?.OrderDate.Month, customer.Orders.Sum(o => o.Total) descending, customer.CustomerID
                                  select customer;

            ObjectDumper.Write(sortedCustomers, 2);
        }
        ...
        public void Linq5()
        {
            var customersGroup = from customer in GetCustomerList()
                                 where customer.Address.All(s => s < '0' || s > '9') || string.IsNullOrEmpty(customer.Region) || customer.Phone.Contains('(') && customer.Phone.Contains(')')
                                 select customer;

            ObjectDumper.Write(customersGroup);
        }
        ...
        public void Linq6()
        {
            var productGroup = from product in GetProductList()
                               ...
                               select new
                               {
                                   ...
                                   PriceGroup =
                               from product in unitsInStockGroup
                               group product by product.UnitPrice into priceGroup
                               select new { Price = priceGroup.Key, Product = priceGroup }
                               }
                               };

            ObjectDumper.Write(productGroup, 3);
        }
        ...
        public void Linq8()
        {
            ...
            var averageOrderAmountForAllCustomersFromGivenCity = GetCustomerList().Where(c => c.City == city).Average(c => c.Orders.Sum(o => o.Total));
            var customersFromCity = GetCustomerList().Where(c => c.City == city);

            //var averageNumberOfOrdersPerCustomerFromEachCity = GetCustomerList().GroupBy(c => c.City).Select(c => c.Average(o => o.Orders.Count()));

            var averageNumberOfOrdersPerCustomerFromEachCity = from customer in GetCustomerList()
                                                               group customer by customer.City into customerCityGroup
                                                               select new { average = customerCityGroup.Average(c => c.Orders.Count()), customerCityGroup.Key, customerCityGroup };

            ObjectDumper.Write(averageNumberOfOrdersPerCustomerFromEachCity, 2);
        }

        [Category("Tasks")]
        [Title("Task9")]
        [Description("Cоставить свои запросы (не менее 5) к наборам данных, описанных классами Product, Customer, Supplier, Order.")]
        public void Linq9()
        {
